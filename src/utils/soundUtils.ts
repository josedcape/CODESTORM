...
/**
 * Configuración por defecto de audio
 */
...
/**
 * Guarda la configuración de audio del usuario
 */
export const saveAudioSettings = (settings: AudioSettings): void => {
  try {
    localStorage.setItem('codestorm-audio-settings', JSON.stringify(settings));
    console.log('💾 Configuración de audio guardada:', settings);
  } catch (error) {
    console.warn('Error al guardar configuración de audio:', error);
  }
};

/**
 * Reproduce el sonido de finalización de proceso si está habilitado
 */
export const playCompletionSound = async (processName?: string): Promise<boolean> => {
  const settings = getAudioSettings();

  if (!settings.enabled || !settings.completionSoundEnabled) {
    console.log('🔇 Sonido de finalización deshabilitado por configuración del usuario');
    return false;
  }

  console.log(`🎉 Reproduciendo sonido de finalización${processName ? ` para ${processName}` : ''}...`);
  const result = await playSound('futur.mp3', settings.volume);

  if (result) {
    console.log('✅ Sonido de finalización reproducido correctamente');
  } else {
