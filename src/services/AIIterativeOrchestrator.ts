...
export class AIIterativeOrchestrator {
  ...
  private processApprovedFiles(approvedItems: ApprovalItem[]): void {
    try {
      ...
      // Limpiar datos temporales del localStorage después de la generación exitosa
  ...
  public async processUserInstruction(instruction: string, templateId?: string): Promise<void> {
    try {
      ...
      // Recuperar el stack tecnológico seleccionado del localStorage
      let selectedTechnologyStack = null;
      try {
        const stackData = localStorage.getItem('selectedTechnologyStack');
        if (stackData) {
          selectedTechnologyStack = JSON.parse(stackData);
          console.log('📦 Stack tecnológico recuperado:', selectedTechnologyStack);

          // Añadir información del stack al mensaje del AgenteLector
          this.addChatMessage({
            id: generateUniqueId('msg-stack-info'),
            sender: 'ai-agent',
            content: `🛠️ **AgenteLector**: Utilizando el stack tecnológico ${selectedTechnologyStack.name} con las tecnologías: ${selectedTechnologyStack.technologies.join(', ')}. Esto optimizará la generación de código para ${selectedTechnologyStack.useCase}.`,
            timestamp: Date.now(),
            type: 'agent-report',
            metadata: {
              agentType: 'lector',
              phase: 'planning',
              technologyStack: selectedTechnologyStack
            }
          });
        }
      } catch (error) {
        console.warn('Error al recuperar el stack tecnológico:', error);
      }
  ...
  /**
   * Limpia los datos temporales del localStorage después de una generación exitosa
   */
  private cleanupTemporaryData(): void {
    try {
      // Limpiar datos de la sesión de construcción
      localStorage.removeItem('originalInstruction');
      localStorage.removeItem('finalInstruction');
      localStorage.removeItem('selectedTechnologyStack');

      console.log('🧹 Datos temporales del localStorage limpiados exitosamente');
    } catch (error) {
      console.warn('Error al limpiar datos temporales del localStorage:', error);
    }
  }
