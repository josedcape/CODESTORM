...
export class EnhancedAPIService {
  ...
  private async testOpenAIConnection(): Promise<APIResponse> {
    try {
      ...
      const response = await this.makeRequestWithRetry('/api/openai/v1/chat/completions', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
  ...
  private async makeRequestWithRetry(
    ...
    try {
      const response = await fetch(url, {
        ...options,
        signal: AbortSignal.timeout(120000) // 2 minute timeout para operaciones complejas
      });
