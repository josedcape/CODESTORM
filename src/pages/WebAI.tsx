...
const WebAI: React.FC = () => {
  ...
  // Completion sounds
  const { playWebAIComplete } = useCompletionSounds();
    ...
    try {
      ...
      if (result.success && result.files.length > 0) {
        ...
        if (htmlFile) {
          ...
          // Reproducir sonido de finalización de WebAI
          playWebAIComplete().then(success => {
            if (success) {
              console.log('🔊 Sonido de finalización de WebAI reproducido');
            }
          });
