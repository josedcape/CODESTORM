...
/**
 * NextEditPanel - Componente de vista previa de cambios con scroll mejorado
 *
 * Correcciones implementadas:
 * - Estructura flex mejorada para evitar problemas de altura
 * - Scroll vertical funcional en todas las secciones
 * - Botones de acción siempre visibles (sticky footer)
 * - Responsive design para diferentes tamaños de pantalla
 * - Manejo adecuado del overflow en comparaciones de código
 */
...
.home-container .reset-btn:hover {
    ...
    if (previewMode === 'side-by-side') {
      return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div>
            ...
            <div className="bg-codestorm-darker rounded border border-red-500/30 p-3 text-xs font-mono max-h-80 overflow-y-auto custom-scrollbar">
          ...
          <div>
            ...
            <div className="bg-codestorm-darker rounded border border-green-500/30 p-3 text-xs font-mono max-h-80 overflow-y-auto custom-scrollbar">
      ...
      return (
        <div>
          ...
          <div className="bg-codestorm-darker rounded border border-codestorm-blue/30 p-3 text-xs font-mono max-h-96 overflow-y-auto custom-scrollbar">
            {/* Simplificado: mostrar solo las líneas propuestas con indicadores */}
            {proposedLines.map((line, index) => {
              const isNew = !originalLines.includes(line);
              return (
                <div key={index} className={`${isNew ? 'bg-green-500/20 text-green-300' : 'text-gray-300'} whitespace-pre-wrap break-words`}>
                  <span className="text-gray-500 mr-3 select-none">{index + 1}</span>
  ...
  return (
    <div className="h-full bg-codestorm-dark rounded-lg overflow-hidden flex flex-col preview-panel">
      {/* Header */}
      <div className="flex-shrink-0 p-4 border-b border-codestorm-blue/30">
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className="w-1/3 border-r border-codestorm-blue/30 flex flex-col">
        ...
        <div className="flex-1 flex flex-col min-h-0">
          {selectedModification ? (
            <div className="flex flex-col h-full">
              {/* Información del cambio - Fixed header */}
              <div className="flex-shrink-0 p-4 border-b border-codestorm-blue/30">
              ...
              {/* Comparación de código - Scrollable content */}
              <div className="flex-1 overflow-y-auto custom-scrollbar p-4 preview-content">
                {renderCodeComparison(selectedModification)}
              </div>

              {/* Botones de acción - Fixed footer */}
              <div className="flex-shrink-0 p-4 preview-footer action-buttons">
                <div className="flex items-center justify-center space-x-4">
                  <button
                    onClick={() => onReject(selectedModification.id)}
                    className="flex items-center space-x-2 px-6 py-3 bg-red-500/20 text-red-400 border border-red-500/30 rounded-lg hover:bg-red-500/30 transition-colors font-medium shadow-lg"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center space-x-2 px-6 py-3 bg-green-500/20 text-green-400 border border-green-500/30 rounded-lg hover:bg-green-500/30 transition-colors font-medium shadow-lg"
                  >
