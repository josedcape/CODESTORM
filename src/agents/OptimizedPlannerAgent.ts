    ...
    try {
      ...
      for (let attempt = 1; attempt <= this.MAX_RETRIES; attempt++) {
        try {
          ...
          // Procesar instrucción con timeout y fallback automático
          const response = await Promise.race([
            tryWithFallback(prompt, 'Gemini 2.5'),
          ...
          const parsedData = this.parseAndValidateResponse(response.content);
          ...
          // Si es un error de cuota y ya se usó fallback, no reintentar inmediatamente
          if (lastError.message.includes('429') || lastError.message.includes('quota')) {
            console.log(`[OptimizedPlannerAgent] Error de cuota detectado. El sistema de fallback ya se ejecutó.`);

            if (attempt < this.MAX_RETRIES) {
              // Esperar más tiempo para errores de cuota (14 segundos como sugiere la API)
              console.log(`[OptimizedPlannerAgent] Esperando 15 segundos antes del siguiente intento...`);
              await new Promise(resolve => setTimeout(resolve, 15000));
            }
          } else if (attempt < this.MAX_RETRIES) {
            // Esperar tiempo normal para otros errores
