...
export class FileObserverAgent {
  ...
  private static generateFileDescription(
    ...
    // Añadir tipo de archivo (verificar que language existe)
    const language = file.language || 'desconocido';
    parts.push(`Archivo ${language.toUpperCase()}`);
  ...
  private static detectCodePatterns(
    ...
    // Detectar patrones según el lenguaje (verificar que language existe)
  ...
  private static generateSuggestions(
    ...
    // Sugerencias según el lenguaje (verificar que language existe)
    const language = file.language || '';
    switch (language) {
      case 'javascript':
      case 'typescript':
        ...
        if (language === 'javascript' && (context.classes.length > 2 || context.functions.length > 5)) {
          observations.push({
            id: `obs-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
            fileId: file.id,
