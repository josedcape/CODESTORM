...
import { voiceRecognitionService, VoiceRecognitionSettings } from '../../services/VoiceRecognitionService';
...
const AudioControls: React.FC<AudioControlsProps> = ({
  ...
  const [voiceSettings, setVoiceSettings] = useState<VoiceRecognitionSettings>(voiceRecognitionService.getSettings());
  ...
  useEffect(() => {
    ...
    const voiceListener = () => {
      setIsListening(voiceRecognitionService.isCurrentlyListening());
    };

    voiceRecognitionService.addListener(voiceListener);

    return () => {
      clearInterval(checkSpeaking);
      voiceRecognitionService.removeListener(voiceListener);
    };
  }, []);
  ...
  const activateStorm = () => {
    voiceRecognitionService.activateStormCommand();
  };
