...
const DirectoryExplorer: React.FC<DirectoryExplorerProps> = ({
  ...
  const directoryTree = useMemo(() => {
    ...
    files.forEach(file => {
      ...
      // Validar que se encontró una barra
      if (lastSlashIndex === -1) {
        // Si no hay barra, es un archivo en la raíz
        const directoryNode = findOrCreateDirectory('/');

        // Verificar si el archivo ya existe en el directorio
        const existingFile = directoryNode.children.find(child =>
          !child.isDirectory && child.name === filePath
        );
      ...
      if (!fileName) {
        console.warn('Nombre de archivo vacío para la ruta:', filePath);
        return;
      }
