...
const [isEnhancing, setIsEnhancing] = useState(false);
  ...
  const handleUseEnhancedPrompt = () => {
    if (currentEnhancedPrompt) {
      // Guardar la instrucción mejorada en localStorage para el flujo de stack selection
      localStorage.setItem('enhancedInstruction', currentEnhancedPrompt.enhancedPrompt);
  ...
  // Función para continuar con la selección de tecnología después de mejorar el prompt
  const handleContinueToStackSelection = () => {
    if (currentEnhancedPrompt) {
      // Guardar tanto la instrucción original como la mejorada
      localStorage.setItem('originalInstruction', currentEnhancedPrompt.originalPrompt);
      localStorage.setItem('enhancedInstruction', currentEnhancedPrompt.enhancedPrompt);
      ...
      // Procesar la instrucción mejorada para mostrar el selector de stack
      // Esto activará el flujo normal que muestra el selector de tecnología
      onSendMessage(currentEnhancedPrompt.enhancedPrompt);
    }
  };
    ...
    `}>
      ...
      {currentEnhancedPrompt && (
        <EnhancedPromptDialog
          ...
          onSelectTechnology={handleContinueToStackSelection}
        />
      )}
