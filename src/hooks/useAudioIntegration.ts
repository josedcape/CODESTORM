...
import { voiceRecognitionService, VoiceRecognitionEvent } from '../services/VoiceRecognitionService';
...
export const useAudioIntegration = (
  ...
  useEffect(() => {
      ...
      switch (event.type) {
        case 'start':
          setState(prev => ({
            ...prev,
            isListening: true,
            isStormMode: voiceRecognitionService.isStormMode()
          }));
          break;
    ...
    if (enableVoiceRecognition) {
      voiceRecognitionService.addListener(handleVoiceEvent);
    }

    return () => {
      if (enableVoiceRecognition) {
        voiceRecognitionService.removeListener(handleVoiceEvent);
      }
    };
