...
@app.route('/explorer_simple')
def explorer_simple():
    """File explorer view with ultra-minimal UI."""
    try:
        user_id = session.get('user_id', 'default')
        workspace_path = get_user_workspace(user_id)
        ...
        if os.path.isdir(target_path):
            ...
            return render_template('explorer_ultra_simple.html', 
                                  items=items, 
...
@app.route('/api/explorer/create_file_simple', methods=['POST'])
def explorer_create_file_simple():
    """Create a file in the workspace - ultra simple form version."""
    try:
        path = request.form.get('path', '.')
        file_name = request.form.get('file_name', '')
        file_content = request.form.get('file_content', '')
        ...
        if not file_name:
            return redirect(f'/explorer_simple?path={path}&error=Nombre+de+archivo+requerido')
        ...
        return redirect(f'/explorer_simple?path={path}')
        
    except Exception as e:
        logging.error(f"Error al crear archivo: {str(e)}")
        return redirect(f'/explorer_simple?error=Error+al+crear+archivo')
        
@app.route('/api/explorer/create_folder_simple', methods=['POST'])
def explorer_create_folder_simple():
    """Create a folder in the workspace - ultra simple form version."""
    try:
        path = request.form.get('path', '.')
        folder_name = request.form.get('folder_name', '')
        
        if not folder_name:
            return redirect(f'/explorer_simple?path={path}&error=Nombre+de+carpeta+requerido')
            
        user_id = session.get('user_id', 'default')
        workspace_path = get_user_workspace(user_id)
        
        # Construir la ruta completa de la carpeta
        folder_path = os.path.join(path, folder_name) if path != '.' else folder_name
        folder_path = folder_path.replace('..', '')
        target_folder = os.path.join(workspace_path, folder_path)
        ...
        logging.error(f"Error al crear carpeta: {str(e)}")
        return redirect(f'/explorer_simple?error=Error+al+crear+carpeta')

@app.route('/api/explorer/delete_simple', methods=['POST'])
def explorer_delete_file_simple():
    """Delete a file or directory in the workspace - ultra simple form version."""
    try:
        path = request.form.get('path')
        if not path:
            return redirect('/explorer_simple?error=No+se+especificó+ruta')
        ...
        # Verificar que existe
        if not os.path.exists(item_path):
            return redirect(f'/explorer_simple?error=El+elemento+no+existe')
            
        # Determinar carpeta contenedora antes de eliminar
        parent_dir = os.path.dirname(path) if path != '.' else '.'
        ...
        try:
            ...
            # Redirigir a la carpeta contenedora
            return redirect(f'/explorer_simple?path={parent_dir}')
        except Exception as e:
            logging.error(f"Error al eliminar {path}: {str(e)}")
            return redirect(f'/explorer_simple?error=Error+al+eliminar:+{str(e)}')
